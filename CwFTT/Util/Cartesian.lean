import Mathlib.CategoryTheory.Monoidal.Closed.Cartesian
universe v u
namespace CategoryTheory
variable {C : Type u} [Category.{v} C] [CartesianMonoidalCategory C] [MonoidalClosed C]
open Limits MonoidalCategory CartesianMonoidalCategory MonoidalClosed

-- @[simp]
lemma internalHom.obj_obj (X Y : C) :
  (MonoidalClosed.internalHom.obj (.op X)).obj Y = (ihom X).obj Y := rfl

-- @[simp]
lemma internalHom.obj_map (X Y Z : C) (f : Y ⟶ Z) :
  (MonoidalClosed.internalHom.obj (.op X)).map f = (ihom X).map f := rfl

-- @[simp]
lemma uncurry_internalHom_map_app (X Y Z : C) (f : Y ⟶ Z) :
    uncurry ((MonoidalClosed.internalHom.map f.op).app X) = f ▷
      (MonoidalClosed.internalHom.obj _).obj _ ≫ (ihom.ev _).app X := by
  exact MonoidalClosed.uncurry_pre _ _

instance (X Y Z : C) (f : X ⟶ Y) [Mono f] : Mono (f ▷ Z) where
  right_cancellation {Z'} g h heq := by
    refine hom_ext g h ?_ ?_
    · apply Mono.right_cancellation (f := f)
      simp only [Category.assoc,← whiskerRight_fst,reassoc_of% heq]
    · simpa using congr($heq ≫ CartesianMonoidalCategory.snd _ _)

instance (X Y Z : C) (f : X ⟶ Y) [Mono f]: Mono (Z ◁ f) where
  right_cancellation {Z'} g h heq := by
    refine hom_ext g h ?_ ?_
    · simpa using congr($heq ≫ CartesianMonoidalCategory.fst _ _)
    · apply Mono.right_cancellation (f := f)
      simp only [Category.assoc,← whiskerLeft_snd,reassoc_of% heq]

-- there is a one-line proof which (ab)uses defeq, but this seems better.
-- @[simp]
lemma internalHom.map_app_eq (X Y Z : C) (f : Y ⟶ Z) :
    (MonoidalClosed.internalHom.map f.op).app X = curry (f ▷
      (MonoidalClosed.internalHom.obj _).obj _ ≫ (ihom.ev _).app X) := by
  apply MonoidalClosed.uncurry_injective
  rw [uncurry_curry,uncurry_internalHom_map_app]

/--
naturality is shown by uncurrying, then showing that the following diagram commutes:
https://q.uiver.app/#q=
-/
@[simps]
def MonoidalClosed.internalHom.unit [CartesianMonoidalCategory C] [BraidedCategory C]
    [MonoidalClosed C] (X : C) :
    𝟭 C ⟶ (internalHom.flip.obj X ⋙ opOp C).unop ⋙
      (internalHom.flip.obj X) where
  app Y :=
    curry <| (β_ _ Y).hom ≫ ((ihom.ev _).app _)
  naturality {Y₁ Y₂} f := by
    simp only [Functor.id_obj, Functor.comp_obj, Functor.unop_obj, Functor.flip_obj_obj, opOp_obj,
      Functor.id_map, Functor.comp_map, Functor.unop_map, Functor.flip_obj_map, opOp_map,
      Quiver.Hom.unop_op]
    apply uncurry_injective
    rw [uncurry_natural_left, uncurry_curry,
      BraidedCategory.braiding_naturality_right_assoc, ← uncurry_internalHom_map_app, uncurry_eq,
      ← BraidedCategory.braiding_naturality_left_assoc ((internalHom.map f.op).app X),
      uncurry_natural_left, uncurry_internalHom_map_app, MonoidalCategory.whisker_exchange_assoc,
      ← uncurry_eq, uncurry_curry]

@[simps]
def MonoidalClosed.internalHom.counit [CartesianMonoidalCategory C] [BraidedCategory C]
    [MonoidalClosed C] (X : C) :
    (internalHom.flip.obj X) ⋙
      (internalHom.flip.obj X ⋙ opOp C).unop ⟶ 𝟭 Cᵒᵖ where
  app Z := ((internalHom.unit X).app (Z.unop)).op
  naturality {Y₁ Y₂} f := by
    rw [Functor.comp_map,Functor.unop_map,Functor.comp_map]
    rw [opOp_map]
    simp only [Functor.id_obj, Functor.flip_obj_map, Quiver.Hom.unop_op, Functor.id_map]
    exact congr($((internalHom.unit X).naturality f.unop).op).symm

-- #check CartesianClosed.internalHom.counit

/--
the proof is by showing the following diagram commutes:
https://q.uiver.app/#q=
-/
lemma MonoidalClosed.internalHom.right_triangle_components
    [BraidedCategory C] (X Y : C) :
    (internalHom.unit X).app ((MonoidalClosed.internalHom.obj (Opposite.op Y)).obj X) ≫
    (MonoidalClosed.internalHom.map ((internalHom.counit X).app _)).app _ = 𝟙 _ := by
  simp only [Functor.id_obj, Functor.comp_obj, Functor.unop_obj, Functor.flip_obj_obj, opOp_obj,
    unit_app, counit_app]
  apply uncurry_injective
  rw [uncurry_natural_left, uncurry_internalHom_map_app,
    whisker_exchange_assoc, ← uncurry_eq, uncurry_curry,
    BraidedCategory.braiding_naturality_left_assoc,← uncurry_eq, uncurry_curry,
    SymmetricCategory.symmetry_assoc]
  simp [uncurry_id_eq_ev]

def MonoidalClosed.internalHom.flip_adjoint
    [BraidedCategory C] (X : C) :
    (internalHom.flip.obj X ⋙ opOp C).unop ⊣ (internalHom.flip.obj X) where
  unit := internalHom.unit X
  counit := internalHom.counit X
  left_triangle_components Y := congr($(internalHom.right_triangle_components X Y).op)
  right_triangle_components Y := internalHom.right_triangle_components X Y.unop


abbrev CartesianMonoidalCategory.graph {X Y : C} (f : X ⟶ Y) :
    X ⟶ (X ⊗ Y) := CartesianMonoidalCategory.lift (𝟙 _) f

abbrev CartesianMonoidalCategory.graph' {X Y : C} (f : X ⟶ Y) :
    X ⟶ (Y ⊗ X) := CartesianMonoidalCategory.lift f (𝟙 _)

omit [MonoidalClosed C] in
lemma CartesianMonoidalCategory.graph_braiding [BraidedCategory C]
    {X Y : C} (f : X ⟶ Y) : graph f ≫ (β_ _ _).hom = graph' f := by
  simp [graph',graph]

end CategoryTheory
